...
--Sprint 45
--Se agrega la tabla para recibir la estructura del archivo de excel
--https://gitlab.com/idiger-analisis/sab-reportes/-/issues/38
--12/07/2023
CREATE TABLE PROMEDIOSHISTORICOS 
(
  ESTACION VARCHAR2(70) 
, IDSENSOR NUMBER 
, ENERO NUMBER 
, FEBRERO NUMBER 
, MARZO NUMBER 
...
, DICIEMBRE NUMBER 
, ANUAL NUMBER 
, PERIODOREFERENCIA VARCHAR2(20) 
, NANIOSCOMPLETOS NUMBER 
, OBSERVACIONES VARCHAR2(200) 
);


CREATE TABLE MAXIMOSMENSUALES 
(
  ESTACION VARCHAR2(70 BYTE) 
, IDSENSOR NUMBER 
, ENERO NUMBER 
, FEBRERO NUMBER 
, MARZO NUMBER 
...
, DICIEMBRE NUMBER 
, ANUAL NUMBER 
, PERIODOREFERENCIA VARCHAR2(20 BYTE) 
, NANIOSCOMPLETOS NUMBER 
...
--Sprint 45
--Se agrega un nuevo reporte Acumulado diario de lluvias - Día Meteorológico, de todas las estaciones e individuales 
--https://gitlab.com/idiger-analisis/sab-reportes/-/issues/40
--17/07/2023
--Procedimiento PA_ACUMULADOS para todas las estaciones
create or replace PROCEDURE PA_ACUMULADOS  
(
P_TIPOSENSOR SENSORES.IDTIPOSENSOR%type,
P_TIPOREPORTE NUMBER,
VFECHAINICIO date,
...
AS
PIVOT_REPORTES VARCHAR2(4000);
SQL_MVACUMULADOS VARCHAR2(4000);
HORARIO_METEREOLOGICO VARCHAR(4000);
BEGIN
 SELECT LISTAGG('''' || E.ESTACION || '''', ',') WITHIN GROUP (ORDER BY E.ESTACION )  INTO PIVOT_REPORTES
    FROM   ESTACIONES E
    INNER JOIN SENSORES S ON E.IDESTACION=S.IDESTACION
    WHERE S.IDTIPOSENSOR=P_TIPOSENSOR AND E.IDPROPIETARIO !=7  AND  S.MOSTRAR=1;
IF P_TIPOREPORTE=71 THEN
 HORARIO_METEREOLOGICO:=' AND TO_CHAR(L.FECHALECTURA, ''HH24'') >= ''07'' AND TO_CHAR(L.FECHALECTURA, ''HH24'') < ''19'' ';
ELSE 
  HORARIO_METEREOLOGICO:='';
END IF;
    
SQL_MVACUMULADOS:='SELECT *
       FROM (SELECT E.ESTACION,TRUNC(L.FECHALECTURA)FECHALECTURA,L.VALORLECTURA
            FROM SENSORES S
            INNER JOIN ESTACIONES E ON E.IDESTACION=S.IDESTACION
            INNER JOIN LECTURAS L ON L.IDSENSOR=S.IDSENSOR
            INNER JOIN TIPOSSENSORES TS ON S.IDTIPOSENSOR = TS.IDTIPOSENSOR 
            WHERE S.IDTIPOSENSOR='|| P_TIPOSENSOR ||' AND E.IDTIPOESTACION=1 AND S.MOSTRAR=1 '||HORARIO_METEREOLOGICO ||'
            AND  (FECHALECTURA) >= (to_date (''' ||  VFECHAINICIO || ''',''dd/mm/yy'')) AND (FECHALECTURA) <(to_date('''||VFECHAFIN|| ''',''dd/mm/yy''))+1) ESTA
            PIVOT (SUM(VALORLECTURA) FOR ESTACION IN
             (' || PIVOT_REPORTES || ')) ORDER BY  FECHALECTURA';
    dbms_output.put_line(SQL_MVACUMULADOS);
 OPEN P_SALIDA FOR
    SQL_MVACUMULADOS;
END PA_ACUMULADOS;
